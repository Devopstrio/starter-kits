...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Kits Catalog', path: '/', icon: LayoutDashboard },
    { name: 'Project Generator', path: '/generator', icon: Zap },
    { name: 'Template Library', path: '/templates', icon: Box },
    { name: 'CLI Tooling', path: '/cli', icon: Terminal },
    { name: 'Governance', path: '/governance', icon: ShieldCheck },
    { name: 'Release History', path: '/releases', icon: History },
    { name: 'Cloud Blueprints', path: '/cloud', icon: Cloud },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-violet-400 bg-clip-text text-transparent">STARTER-KITS</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 truncate">Template Master</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search starter kits, tech stacks, or templates..." 
                className="w-full bg-slate-800/50 border border-slate-700 rounded-lg py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-violet-600/50 focus:border-violet-600/50 transition-all"
              />
            </div>
          </div>
